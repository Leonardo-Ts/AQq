...
public class MovilRestWebService  implements MovilRestWebServiceInterfase , InitializingBean{
	        ...
	        if (term.getReporteSac() != null) {
				...
				try {
					this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term,
						term.getReporteSac().toString(), this.fuenteWS + "insertarArribo",
...
//							Date fechaArribo = new Date();
		...
		//*** ACTUALIZADO ***
		@Override
		public GETMovilResultadoOperacion insertarTermino(GETMovilServicioTermino serv) {
			  if (serv == null) {
		        ...
		        if (term.getReporteSac() != null) {
					reporteSac = term.getReporteSac().toString();
				}
		        // Se graba lo que llega
		        try {
...
//						if(StringUtils.containsIgnoreCase(respuestaSac, "OK")) {
		...
		@Override
		public GETMovilResultadoOperacion insertarGestion( GETMovilServicioGestion serv) {
			
			 if (serv == null) {
		            return new GETMovilResultadoOperacion(false, "ERROR: El objeto gestion es nulo");
		        }
		        if (StringUtils.isBlank(serv.getUsuario())) {
		            return new GETMovilResultadoOperacion(false, "ERROR: El campo 'usuario' de gestion es nulo");
		        }
		        if (StringUtils.isBlank(serv.getPasswd())) {
		            return new GETMovilResultadoOperacion(false, "ERROR: El campo 'passwd' de gestion es nulo");
		        }
		        if (serv.getDatosAsegurado() == null) {
		            return new GETMovilResultadoOperacion(false, "ERROR: El campo 'Datos Asegurado' de gestion es nulo");
		        }
...
//		            int terceros = 1;
		            if ((serv.getDatosTercero() != null) && (serv.getDatosTercero().length > 0)) {
		                for (final MovilDatosTercero dat : serv.getDatosTercero()) {
							...
							if (!StringUtils.isBlank(dat.getVehiculoPlacas()) || dat.getAtropello() || !StringUtils.isBlank(dat.getTerceroSerie())) {

								if (dat.getConsecutivoTercero().trim().equals("")) {
									return new GETMovilResultadoOperacion(false, "ERROR: No se asignó un Id de Asegurado o Tercero");
								}
								...
								if(dat.getAtropello() == null || !dat.getAtropello()) {
									if(dat.getVehiculoModelo().length() == 4){
										if(Integer.parseInt(dat.getVehiculoModelo()) >= 1800 && Integer.parseInt(dat.getVehiculoModelo()) <= currentYear){
											...
											return new GETMovilResultadoOperacion(false, "ERROR: Modelo Vehiculo fuera de rango (1800 - " + currentYear + ").");
										}
									}else{
										return new GETMovilResultadoOperacion(false, "ERROR: Modelo Vehiculo debe contener 4 digitos.");
									}
								}
								char[] caracteresEspeciales = {'\u00A1','\u0021','\u0022','\u0023','\u0024','\u0025','\u0026','\u002F','\u0028','\u0029','\u003D','\u003F','\u00BF','\u007B','\u007D','\u005B','\u005D','\u003A','\u002E','\u002D','\u005F','\u002A','\u002B', '\u05F3', '\u00B4', ' '};
								...
								if (BooleanUtils.isFalse(dat.getAtropello()) || dat.getAtropello() == null) {
									...
									if (StringUtils.defaultString(dat.getTerceroSerie()) != null && StringUtils.defaultString(dat.getTerceroSerie()).length() == 17) {
										...
										return new GETMovilResultadoOperacion(false, "ERROR: El campo 'Serie Tercero' debe ser de 17 digitos");
									}
								}
...
//								terceros++;
		         ...
		         // Los datos del objeto
		            if ((serv.getDatosObjeto() != null) && (serv.getDatosObjeto().length > 0)) {
		                for (final MovilDatosObjeto dat : serv.getDatosObjeto()) {
		                    if (StringUtils.isNotBlank(dat.getBache()) || StringUtils.isNotBlank(dat.getMalla())
		                        ...
		                        dgt.setNumero(dat.getConsecutivoTerceroObjeto());
...
//		                        terceros++;
		            ...
		            Map<String, String> respuestaSac = null;
		            try {
		                this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term, datos.getReporte(),
		                ...
		                return new GETMovilResultadoOperacion(false, "ERROR: SAC Excepciona: " + e.getMessage());
		            }

		            try {
		                this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term, datos.getReporte(), // Manda a guardar con el SP
		...
		@Override
		public GETMovilResultadoInicioDeSession solicitarInicioDeSesion(GETMovilInicioDeSesion serv) {
			 if (serv == null) {
		            return new GETMovilResultadoInicioDeSession(false, "ERROR: Es necesario especificar las credenciales");
		        }
		        if (StringUtils.isBlank(serv.getUsuario())) {
		            return new GETMovilResultadoInicioDeSession(false, "ERROR: Es necesario especificar un usuario");
		        }
		        if (StringUtils.isBlank(serv.getPasswd())) {
		            return new GETMovilResultadoInicioDeSession(false, "ERROR: Es necesario especificar una contrasena");
		        }
		        ...
		        if (term == null) {
		            return new GETMovilResultadoInicioDeSession(false, "ERROR: El usuario y contrasena indicados no son validos.");
		        }
		        ...
		        try {
			       ...
			       if(lista.contains(serv.getVersion())) {} else {
			    	 ...
			    	 return new GETMovilResultadoInicioDeSession(false, "ADVERTENCIA: La versión no está actualizada, no es posible iniciar sesión.");
		       ...
		       //----- 
		        ...
		        if (!StringUtils.equalsIgnoreCase(term.getEstatus(), "disponible")
		            ...
		            try {
		                this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(
		                        ...
		                        term,
		                        null,
		                        this.fuenteWS + "solicitarInicioDeSesion",
		            ...
		            return new GETMovilResultadoInicioDeSession(false, "Inicio de Sesion Rechazada. Su estatus es: "
		                    + term.getEstatus());
		        ...
		        if (term.getReporteSise() != null) {
		            ...
		            return new GETMovilResultadoInicioDeSession(true, "OK", lc);
		        ...
		        try {
		            this.historicoTerminalServiceInterfase
		                    .ejecutarFuncionHistoricoTerminalNuevo(null, term, null, this.fuenteWS + "solicitarInicioDeSesion",
		                            "Inicio de Sesion Movil V3",
		                            "Se inicio sesion y se puso en disponible de forma NORMAL (por fin) por que no tiene reportes pendientes");
		        } catch (final Exception ex) {
		            this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
		                    "solicitarInicioDeSesion => ejecutarFuncionHistoricoTerminalNuevo");
		        }
		        ...
		        try {
		            ...
		            return new GETMovilResultadoInicioDeSession(r.isExito(), r.getMensaje(), lc);
		        } catch (final Exception ex) {
		            this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
		                    "solicitarInicioDeSesion => ejecutarFuncionTerminalEstatusDisponible");
		            return new GETMovilResultadoInicioDeSession(false, ex.getMessage());
		...
		@Override
		public GETMovilReporte solicitarDatosDeReporte(GETMovilInicioDeSesion serv) {
	    	boolean esAjustadorVulnerable = false;

			 if (serv == null) {
		            return new GETMovilReporte("ERROR: Es necesario especificar las credenciales", null, null, esAjustadorVulnerable);
		        }
		        if (StringUtils.isBlank(serv.getUsuario())) {
		            return new GETMovilReporte("ERROR: Es necesario especificar un usuario", null, null, esAjustadorVulnerable);
		        }
		        if (StringUtils.isBlank(serv.getPasswd())) {
		            return new GETMovilReporte("ERROR: Es necesario especificar una contrasena", null, null, esAjustadorVulnerable);
		        }
		        ...
		        if (term == null) {
		            return new GETMovilReporte("ERROR: Usuario o contrasena invalidos", null, null, esAjustadorVulnerable);
		        }
		        ...
		        if (term.getReporteSac() != null) {
					...
					try {
						...
						try{
							...
							if(repSacOut != null){
								//Validacion por ajustadores express
								...
								//Validamos FFHH de pasado ajust y FFHH de ocurrido
								try {
							...
							return new GETMovilReporte("OK", null, repSacOut, esAjustadorVulnerable ); 
		 
						}catch (final Exception ex) {
			                this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
			                        "solicitarDatosDeReporte => ejecutarFuncionReporteSacActualizarFechaReporteLecturaPorWs");
			                return new GETMovilReporte("ERROR: " + ex.getMessage(), null, null, esAjustadorVulnerable);
	...
	public MovilResultadoOp insertarEncuesta(final MovilServicioEncuesta serv) {
		  ...
		  if (StringUtils.isBlank(serv.getNumeroDeReporte())) {
		     return new MovilResultadoOp("ERROR: Es necesario especificar Numero de Reporte");
		    }
		  if (StringUtils.isBlank(serv.getNombreAsegurado())) {
		      return new MovilResultadoOp("ERROR: Es necesario especificar Nombre del Asegurado");
		    }
		   if (serv.getAtencionPersonalCabina()>=5||serv.getAtencionPersonalCabina()==0) {
		      return new MovilResultadoOp("ERROR: VALOR INVALIDO en atencionPersonalCabina!.VALORES VALIDOS 1=mal,2=regular,3=bien,4=excelente");
		  	}
		   if (serv.getLlegadaAjustador()>=5||serv.getLlegadaAjustador()==0) {
		      return new MovilResultadoOp( "ERROR: VALOR INVALIDO en llegadaAjustador!.VALORES VALIDOS 1=mal,2=regular,3=bien,4=excelente");
		      	}

		      	if (serv.getPresentacionAjustador()>=5||serv.getPresentacionAjustador()==0) {
		      		return new MovilResultadoOp("ERROR: VALOR INVALIDO en presentacionAjustador!.VALORES VALIDOS 1=mal,2=regular,3=bien,4=excelente");
		      	}

		      	if (serv.getTratoAjustador()>=5 ) {
		      		return new MovilResultadoOp("ERROR: VALOR INVALIDO en tratoAjustador!.VALORES VALIDOS 0=noAplica, 1=mal,2=regular,3=bien,4=excelente");
		      	}
		      	if (serv.getCapacidadAjustador()>=5||serv.getCapacidadAjustador()==0) {
		      		return new MovilResultadoOp("ERROR: VALOR INVALIDO en capacidadAjustador!.VALORES VALIDOS 1=mal,2=regular,3=bien,4=excelente");
		      	}
		      	if (serv.getTratoAjustadorTercero()>=5) {
		      		return new MovilResultadoOp("ERROR: VALOR INVALIDO en tratoAjustadorTercero!.VALORES VALIDOS 0=noAplica, 1=mal,2=regular,3=bien,4=excelente");
		      	}
		      	if (serv.getServicioDeGrua()>=5) {
		      		return new MovilResultadoOp("ERROR: VALOR INVALIDO en servicioDeGrua!.VALORES VALIDOS 0=noAplica,1=mal,2=regular,3=bien,4=excelente");
		      	}

		      	if (serv.getObservoIrregularidadServicio()==null) {
		      		return new MovilResultadoOp("ERROR: VALOR INVALIDO en ObservoIrregularidadServicio!.VALORES VALIDOS true o false");
		      	}
		       	if (serv.getSeleccionDeTaller()==null) {
		      		return new MovilResultadoOp("ERROR: VALOR INVALIDO en SeleccionDeTaller!.VALORES VALIDOS 0=noAplica, 1=Si, 2=No");
		      	}
		      	if (serv.getRecibioCopiaDA()==null) {
		      		return new MovilResultadoOp("ERROR: VALOR INVALIDO en RecibioCopiaDA!.VALORES VALIDOS true o false");
		      	}
		      	...
		      	// Se graba lo que llega
		      	try {
		      		this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term, serv.getNumeroDeReporte(),
		      	...
		      	try {
		      		...
		      		return new MovilResultadoOp("ERROR: " + "No se pudieron guardar lo Datos. Vuelva a intentarlo por favor.");
		                ...
		                if(term.getReporteSac() != null){
		                    Map<String, Object> respuestaSac = new HashMap<String, Object>();
		                    	...
		                    	if(!respuestaSac.get("salida").toString().contains("ERROR")){
		                    		...
		                    		//****** MANDAR AVISO A SUPERVISOR Y GERENTE DE AJUSTADOR ***** //
		                    		if (cobFlex13) {
		                    			try {
		    ...
		    public MovilResultadoOperacion solicitarCobroBancario(final MovilServicioCobroBancario serv) {
		        ...
		        try {
		            this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(
		                    ...
		                    term,
		                    serv.getReporte(),
		                    this.fuenteWS + "solicitarCobroBancario",
		                    "Cobro Movil V3",
			...
			public MovilResultadoOperacion insertarAbogado(final MovilServicioAbogado serv) {
				...
				try {
					this.sessionExternaDao.informacionDeSessionExterna(term, "Solicitud de abogado");
				} catch (final Exception ex) {
					this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
							"insertarAbogado => informacionDeSessionExterna");
				}
				...
				ReporteAbogado reporteUno = null;
				try {
					reporteUno = this.reporteAbogadoDao.objetoReporteAbogadoParaNumeroReporte(reporteAEnviar);
				} catch (final Exception ex) {
					this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
							"insertarAbogado => objetoReporteAbogadoParaNumeroReporte");

				}
				if (reporteUno != null) {
					return new MovilResultadoOperacion(false,
							"ADV: Este reporte ya fue solicitado en la fecha: " + reporteUno.getFechaRecibidoWs());
				}

				final ReporteAbogado reporte = new ReporteAbogado(term, new Date(), reporteAEnviar, null, null, "Abierto",
						serv.getConductorDetenido(), serv.getVehiculoDetenido(), serv.getNumeroLesionadosNA(),
						serv.getNumeroLesionadosTS(), serv.getNumeroMuertosNA(), serv.getNumeroMuertosTS(),
						serv.getLugarAPresentarse(), Integer.parseInt(serv.getResponsabilidad()), serv.getMotivoSolicitud(),null);
				final JMResultadoOperacion resultado = reporte.guardarObjeto();

				
				return new MovilResultadoOperacion(resultado.isExito(), resultado.getMensaje());
...
//		            if (!esEquipoPesado) {
//		                //Dañio menor A deducible
//		                  if ( serv.getDanioMenor() != null ) {
//		                     ap.setDanioMenor(serv.getDanioMenor() ? "1" : "0");            	
//		                    } else {
//		                     	ap.setDanioMenor("0");
//		                   }
//					}
...
//		                        "Solicitud Servicios Diversos V3", "Envio a SAC -> " + entradaSac);
		                ...
		                objetoFolioHospitalAmbulancia.setCorreoTercero(null);
...
//		                	respuestaSac = this.SacSPDao.dispositivoMovilFolioHospitalAmbulancia(objetoFolioHospitalAmbulancia, serv.getClaveDeProveedor(), term);
...
//		            respuestaSise = new JMSISEWebServiceConsultaPort().insertarDatosAsignacionServicio(entradaSise);
		            ...
		            try {
		                ...
		                this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term,
		                        term.getReporteAtendiendo(), this.fuenteWS + "solicitarServiciosDiversos",
		                        "Solicitud Servicios Diversos V3", "SISE MUY MAL !!! SISE EXCEPCIONA!!! -> " + e.getMessage());
				...
				return new MovilResultadoOperacion(false, "La solicitud no pudo ser insertada en SISE. SISE Responde: "
		                + respuestaSise);
		    }
	...
	private void getListasMovilReporte(MovilReporte reporte, ReporteMovilSac reporteSac) {
		    ...
		    public MovilResultadoOperacion insertarCartaCob(final MovilServicioCartaCob serv) {
		        ...
		        if (StringUtils.isBlank(serv.getPassword())) {
		            return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar una contrasena");
		        }

		        Terminal term = null;
		        try {
		            term = this.terminalDao.objetoTerminalParaProveedorYPasswd(serv.getUsuario(), serv.getPassword());
		        } catch (final Exception ex) {
		            this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
		                    "insertarCartaCob => objetoTerminalParaProveedorYPasswd");
		        }
		        ...
		        try {
		            this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term, null,
		                    this.fuenteWS + "insertarCartaCob", "Insertar CartaCob Movil V3", "Ejecucion del Metodo Con Datos -> "
		                            + this.xmlFactory.obtenerString(serv));
		        } catch (final Exception ex) {
		            this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
		                    "insertarCartaCob => ejecutarFuncionHistoricoTerminalNuevo");
		        }

		        if (StringUtils.isBlank(serv.getMarca())) {
		            return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar la marca de vehiculo");
		        }
		        if (StringUtils.isBlank(serv.getModelo())) {
		            return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar el modelo de vehiculo");
		        }
		        if (StringUtils.isBlank(serv.getSerie())) {
		            return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar el numero de serie de vehiculo");
		        }
		        if (serv.getPlacas()== null) {
		            return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar las placas del vehiculo");
		        }
		        if (StringUtils.isBlank(serv.getColor())) {
		            return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar el color de vehiculo");
		        }
		        if (StringUtils.isBlank(serv.getReporte())) {
		            return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar el reporte");
		        }
		        if (StringUtils.isBlank(serv.getTipoPoliza())) {
		            return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar el tipo de Poliza");
		        }
...
//		        	try {
			            ...
			            final JMWSDatosCartaCob cb = new JMWSDatosCartaCob();
			            cb.setReporte(serv.getReporte());
			            cb.setTipoPoliza(serv.getTipoPoliza());
			            cb.setMarca(serv.getMarca());
			            cb.setTipoVehiculo(serv.getTipoVehiculo());
			            cb.setModelo(serv.getModelo());
			            cb.setPlacas(serv.getPlacas());
			            cb.setSerie(serv.getSerie());
			            cb.setColor(serv.getColor());
			            cb.setIdDriver(serv.getIdDriver());
			            cb.setNumCelular(serv.getNumCelular());

		                try {
		                    this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term, null,
		                            this.fuenteWS + "insertaCartaCob", "Insertar CartaCob Movil V3",
		                            "Envio a SAC: " + this.xmlFactory.obtenerString(cb));
		                } catch (final Exception ex) {
		                    this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
		                            "insertarCartaCob => ejecutarFuncionHistoricoTerminalNuevo");
		                }
		                ...
		                try {
							respuestaSac = this.SacSPDao.insertarCuentasEspecialesCartaCob(cb, serv.getUsuario(), serv.getPassword());
						} catch (Exception e) {
							try {
				                this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term, null,
				                        this.fuenteWS + "insertaCartaCob", "Insertar CartaCob Movil V3", "MUY MAL !!! SAC EXCEPCIONA!!! -> "
				                                + e.getMessage());
				            } catch (final Exception ex) {
				                this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
				                        "insertarCartaCob => ejecutarFuncionHistoricoTerminalNuevo");
				            }
				            return new MovilResultadoOperacion(false, "ERROR: Excepcion en SAC: " + e.getMessage());
						}

		                try {
		                    this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term, null,
		                            this.fuenteWS + "insertaCartaCob", "Insertar CartaCob Movil V3", "Respuesta de SAC ->"
		                                    + this.xmlFactory.obtenerString(respuestaSac));
		                } catch (final Exception ex) {
		                    this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
		                            "insertarCartaCob => ejecutarFuncionHistoricoTerminalNuevo");
		                }
		                ...
		                if (respuestaSac == null) {
		                    return new MovilResultadoOperacion(false, "ERROR: Al comunicar con SAC. Respuesta nula");
		                }

		                if (!respuestaSac.contains("ERROR")) {
		                    return new MovilResultadoOperacion(true, "OK: " + respuestaSac);
		                }
		                return new MovilResultadoOperacion(false, "ERROR: " + respuestaSac);

		            } 

		        return new MovilResultadoOperacion(false, "ERROR: Excepcion en Metodo Insertar cartaCob: ");
		    } 
		    ...
		    public MovilResultadoOperacion insertarIdealease(final MovilIdealease serv) {
		        ...
		        try {
		            ...
		            this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
		                    "insertarIdealease => objetoTerminalParaProveedorYPasswd");
		        }
		        ...
		        try {
		            this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term, null,
		                    this.fuenteWS + "insertarIdealease", "Insertar Idealease Movil V3", "Ejecucion del Metodo Con Datos -> "
		                            + this.xmlFactory.obtenerString(serv));
		        } catch (final Exception ex) {
		            this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
		                    "insertarIdealease => ejecutarFuncionHistoricoTerminalNuevo");
		        }

		        if (StringUtils.isBlank(serv.getLicencia())) {
		            return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar el numero de licencia");
		        }
		        ...
		        //SAC
		        if(term.getReporteSac() != null){
		        	String tipo ="IDEALEASE";
		                try {
		                    this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term, null,
		                            this.fuenteWS + "insertarIdealease", "Insertar Idealease Movil V3",
		                            "Envio a SAC: " +"Licencia: "+serv.getLicencia()+" Reporte: "+serv.getReporte() +" Tipo: "+tipo);
		                } catch (final Exception ex) {
		                    this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
		                            "insertarIdealease => ejecutarFuncionHistoricoTerminalNuevo");
		                }
		                ...
		                try {
							respuestaSac = this.SacSPDao.insertarIdealease(serv.getLicencia(), serv.getReporte(), tipo);
						} catch (Exception e) {
							try {
				                this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term, null,
				                        this.fuenteWS + "insertarIdealease", "Insertar Idealease Movil V3", "MUY MAL !!! SAC EXCEPCIONA!!! -> "
				                                + e.getMessage());
				            } catch (final Exception ex) {
				                this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
				                        "insertarIdealease => ejecutarFuncionHistoricoTerminalNuevo");
		                ...
		                try {
		                    this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term, null,
		                            this.fuenteWS + "insertarIdealease", "Insertar Idealease Movil V3", "Respuesta de SAC ->"
		                                    + this.xmlFactory.obtenerString(respuestaSac));
		                } catch (final Exception ex) {
		                    this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
		                            "insertarIdealease => ejecutarFuncionHistoricoTerminalNuevo");
		        ...
		        return new MovilResultadoOperacion(false, "ERROR: Excepcion en Metodo Insertar Idealease: ");
		    } 
...
//		    @Override
		    ...
		    Terminal term = null;
		  	 if ( confirmarGrua == null) {
		           return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar las credenciales");
		       }
		       if (StringUtils.isBlank(confirmarGrua.getUsuario())) {
		           return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar un usuario");
		       }
		       if (StringUtils.isBlank(confirmarGrua.getPass())) {
		           return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar una contrasena");
		       }
		       if (StringUtils.isBlank(confirmarGrua.getReporte())) {
		           return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar un el reporte");
		       }
		       if (StringUtils.isBlank(confirmarGrua.getInvolucrado())) {
		           return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar un involucrado");
		       }
		       if (StringUtils.isBlank(confirmarGrua.getProveedor())) {
		           return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar el ID de proveedor");
		       }
		       
		       if (StringUtils.isBlank(confirmarGrua.getStatus())) {
		           return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar el status");
		       }
		       
		       try {
		           term = this.terminalDao.objetoTerminalParaProveedorYPasswd(confirmarGrua.getUsuario(), confirmarGrua.getPass());
		       } catch (final Exception ex) {
		           this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
		                   "confirmarGrua => objetoTerminalParaProveedorYPasswd");
		       }
		       ...
		       ConfirmarGruaSac confirmarGruaServ = new ConfirmarGruaSac();
		       confirmarGruaServ.setReporte(confirmarGrua.getReporte());
		       confirmarGruaServ.setInvolucrado(confirmarGrua.getInvolucrado());
		       confirmarGruaServ.setProveedor(confirmarGrua.getProveedor());
		       confirmarGruaServ.setStatus(confirmarGrua.getStatus());
		       confirmarGruaServ.setComentarios(confirmarGrua.getComentario());
		       
		       

		       try {
		           this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term, null,
		                   this.fuenteWS + "confirmarGrua", "Confirmar Grua Movil V3",
		                   "Envio a SAC: " + this.xmlFactory.obtenerString(confirmarGruaServ));
		       } catch (final Exception ex) {
		           this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
		                   "confirmarGrua => ejecutarFuncionHistoricoTerminalNuevo");
		       }

		       try {
						respuestaSac = this.SacSPDao.confirmarGrua(confirmarGruaServ);
					} catch (Exception e) {
						try {
			                this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term, null,
			                        this.fuenteWS + "confirmarGrua", "Confirmar Grua Movil V3", "MUY MAL !!! SAC EXCEPCIONA!!! -> "
			                                + e.getMessage());
			            } catch (final Exception ex) {
			                this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
			                        "confirmarGrua => ejecutarFuncionHistoricoTerminalNuevo");
		       ...
		       try {
		           this.historicoTerminalServiceInterfase.ejecutarFuncionHistoricoTerminalNuevo(null, term, null,
		                   this.fuenteWS + "confirmarGrua", "Confirmar Grua Movil V3", "Respuesta de SAC ->"
		                           + this.xmlFactory.obtenerString(respuestaSac));
		       } catch (final Exception ex) {
		           this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
		                   "insertarCartaCob => ejecutarFuncionHistoricoTerminalNuevo");
...
//		       if (StringUtils.isBlank(serv.getTipoAfectado())) {
//		    	   return new MovilResultadoOperacion(false, "ERROR: Es necesario especificar tipoAfectado");
//		       }
//		       if (!serv.getTipoAfectado().equals("A") || !serv.getTipoAfectado().equals("T1") || !serv.getTipoAfectado().equals("T2")) {
//		    	   return new MovilResultadoOperacion(false, "ERROR: El campo tipoAfectado debe ser 'A', 'T1' o 'T2'");
//		       }
...
//			public MovilResultadoOperacion enviarCorreoAvisosAjustador(String usuario, String pwd) {
//				 Terminal term = null;
//			        try {
//			            term = this.terminalDao.objetoTerminalParaProveedorYPasswd(usuario, pwd);
//			        } catch (final Exception ex) {
//			            this.utileriaExcepcion.manejarExcepcion(ex, this.getClass(),
//			                    "insertarArribo => objetoTerminalParaProveedorYPasswd");
//			        }
...
//			        }
//				this.loggerAvisos.info("¡Invoca al metodo Asy con reporte SAC!");
//				this.envioAsyncService.enviarCorreoNuevo(term);
//				return new  MovilResultadoOperacion(true,"Metodo asicrono del envio de mensaje" );
//			}
