...
import com.aaq.col.clases.database.entidades.abstracto.AbstractFormatoAdmisionAutomoviles_;
import com.aaq.col.clases.database.repositorios.SIICAServerGenericDaoJpaImpl;
import com.aaq.col.clases.database.repositorios.interfase.FormatoAdmisionAutomovilesDaoInterfase;
import com.aaq.col.clases.sac.model.DatosInsertarFormatoAdmisionAutomoviles;

@org.springframework.stereotype.Repository(value = "formatoAdmisionAutomovilesDao")
public class FormatoAdmisionAutomovilesDao extends SIICAServerGenericDaoJpaImpl<FormatoAdmisionAutomoviles>
		implements FormatoAdmisionAutomovilesDaoInterfase {
	...
	public FormatoAdmisionAutomoviles objetoFormatoAdmisionAutomoviles(String id) {
		FormatoAdmisionAutomoviles res = null;
		try {
			res = StringUtils.isNotBlank(id)
					? this.getEntityManager().find(FormatoAdmisionAutomoviles.class, new Integer(id))
					: null;

		} catch (final Exception e) {
			log.error("Formatos Error=> objetoFormatoAdmisionAutomoviles=> ", e);
		}
		return res;
	}
	...
	public List<FormatoAdmisionAutomoviles> listaDeFormatoAdmisionAutomoviles() {
		...
		TypedQuery<FormatoAdmisionAutomoviles> typedQ = null;
		try {
			final CriteriaBuilder builder = this.getEntityManager().getCriteriaBuilder();
			final CriteriaQuery<FormatoAdmisionAutomoviles> query = builder
					.createQuery(FormatoAdmisionAutomoviles.class);
			final Root<FormatoAdmisionAutomoviles> root = query.from(FormatoAdmisionAutomoviles.class);
			query.select(root);
			final List<Predicate> predicates = new ArrayList<>();

			predicates.add(builder.or(builder.notEqual(root.get(AbstractFormatoAdmisionAutomoviles_.enviadoFtp), 1),
					builder.isNull(root.get(AbstractFormatoAdmisionAutomoviles_.enviadoFtp))));

			query.where(predicates.toArray(new Predicate[predicates.size()]));
			query.orderBy(builder.desc(root.get(AbstractFormatoAdmisionAutomoviles_.id)));
			typedQ = this.getEntityManager().createQuery(query);

			typedQ.setMaxResults(50);
		} catch (final Exception e) {
			log.error("Formatos Error=> listaDeFormatoAdmisionAutomoviles", e);
	...
	public String ejecutarFuncionInsertarAdmisionAutomoviles(
			...
			Integer oaCarrilExpres, Boolean oaDanioMenor
		...
		try {
			...
			nat.registerStoredProcedureParameter("in_oa_carril_expres", String.class, ParameterMode.IN);
			nat.registerStoredProcedureParameter("in_oa_danio_menor", String.class, ParameterMode.IN);
			...
			nat.setParameter("in_oa_carril_expres", oaCarrilExpres);
			nat.setParameter("in_oa_danio_menor", oaDanioMenor);
			...
			this.documentarExcepcionParaMetodo(e, "ejecutarFuncioninsertarAdmisionAutomoviles", oaTipoAuto,
					oaFolioElectro, oaNumEndoso, oaMarcaAuto, oaEmailCliente, oaDescDanios, oaDeducible, oaNumInciso,
					oaNumPoliza, oaNumReporte, oaTManual, oaPorcentajeDed, oaNumSerie, oaSumaAsegurada,
					oaAdminDeducible, oaNomAjustador, oaTelCliente, oaDaniosPre, oaRazonResponsable, oaNomCliente,
					oaRazonTelefono, oaRazonEnvio, oaPlacaAuto, oaRazonCobertura, oaModeloAuto, oaFecha, oaAsegurado,
					oaKilometraje, oaCantidad, oaTipoDeducible, oaClaveAjustador, oaPerdidaTotal, oaRazonDomicilio,
					oaNumSiniestro, oaColorAuto, oaAgravamiento, oaId, oaNivelInundacion, enviadoEmail, ////
			...
			log.error("Formatos Error=> ejecutarFuncionInsertarAdmisionAutomoviles =>" + oaNumReporte, e);
	...
	@Override
	public String InsertarFormatoAdmisionAutomoviles(DatosInsertarFormatoAdmisionAutomoviles entrada) {
		...
		entry.put("OA_CARRIL_EXPRES", entrada.getOa_carril_expres());
		entry.put("OA_DANIO_MENOR", entrada.getOa_Danio_Menor());
		entry.put("OA_PRODUCTO_ESENCIAL", entrada.getOa_producto_esencial());
		...
		result = ejecutarFuncionWebserviceStoreFAAuto(entry);
		return result;
	}

	// DB
	@Override
	public String ejecutarFuncionWebserviceStoreFAAuto(Map<String, String> entry) {
		StoredProcedureQuery query = this.getEntityManager().createStoredProcedureQuery("WEBSERVICESTORE_FAAuto");
		...
		String[] columnas = new String[] { "OA_FOLIO_ELECTRO", "OA_NUM_REPORTE", "OA_NUM_SINIESTRO", "OA_ASEGURADO",
				...
				"FIRMA_ASEGURADO", "NIU", "OA_CARRIL_EXPRES","OA_DANIO_MENOR","OA_PRODUCTO_ESENCIAL","OA_CODIGO_QR",
				"OA_PT_EVIDENTE","OA_ABANDONO","CORREO_OCULTO","FUENTE_WS","CHECK_5", "CHECK_6"};
		for (int i = 0; i < columnas.length; i++) {
			query.registerStoredProcedureParameter((i + 1), String.class, ParameterMode.IN);
		}
		query.registerStoredProcedureParameter(55, String.class, ParameterMode.OUT);

		for (int x = 0; x < columnas.length; x++) {
			query.setParameter((x + 1), entry.get(columnas[x]));
		}
		query.execute();
		return String.valueOf(query.getOutputParameterValue(55));
	}
	...
	public int obtenerConsecutivo(String reporte) {
		...
		try {
			...
			resultado = Objects.toString(this.getEntityManager()
					.createNativeQuery(
							"select count(*) from FORMATO_ADMISION_AUTOMOVILES where oa_num_reporte='" + reporte + "'")
					.getSingleResult());
